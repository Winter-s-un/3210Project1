...
void EKFSLAM::updateMeasurement(){
    ...
    Ht.block<2, 3>(0, 0) = jacobB(mState, ut, dt);  // Jacobian of the observation model w.r.t. state

    // z.segment 表示从测量向量 z 中提取观测到的圆柱体特征的测距和方位观测
    Eigen::VectorXd z_observed = z.segment<2>(2 * i);
    Eigen::VectorXd z_expected = Eigen::VectorXd::Zero(2);
    z_expected << (landmark - mState.head<2>()).norm(), atan2(landmark(1) - mState(1), landmark(0) - mState(0)) - mState(2);

    Eigen::VectorXd innovation = z_observed - z_expected;
    ...
    for (int i = 0; i < num_obs; ++i) {
        ...
        int associated_landmark_idx = -1;

        for (int j = 0; j < num_landmarks; ++j) {
            const Eigen::Vector2d& predicted_landmark = mState.block<2, 1>(3 + j * 2, 0);
            double distance = (landmark - predicted_landmark).norm();

            if (distance < min_distance) {
                min_distance = distance;
                associated_landmark_idx = j;
            }
        }

        if (associated_landmark_idx != -1) {
            // Update the measurement with the associated landmark
            Eigen::MatrixXd Ht = Eigen::MatrixXd::Zero(2, mState.rows());
            Ht.block(0, 0, 2, 3) = jacobHx(mState, landmark); // Assuming jacobHx is a function that returns the Jacobian of the measurement model

            Eigen::MatrixXd K = mCov * Ht.transpose() * (Ht * mCov * Ht.transpose() + Q).inverse();
            mState += K * (z - h(mState, landmark)); // Assuming h is the measurement model
            mCov = (Eigen::MatrixXd::Identity(mState.rows(), mState.rows()) - K * Ht) * mCov;
        }
